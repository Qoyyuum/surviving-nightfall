...
class GameManager(Entity):
    ...
    def start_game(self):
        ...
        selected_weapon = (
            self.score_system.unlocked_weapons[-1]
            if len(self.score_system.unlocked_weapons) > 0
            else "handgun"
        )
        ...
        self.available_abilities = [ArcLightning(), IceBullets(), Healing()]
    ...
    def resume_game(self):
        ...
        if (
            hasattr(self, "next_wave_to_start")
            and self.next_wave_to_start is not None
    ...
    def handle_collisions(self):
        ...
        hits = self.collision_system.check_projectile_enemy_collisions(
            projectiles, enemies
        )
        ...
        if hasattr(self.player.current_weapon, "projectiles"):
            for projectile in self.player.current_weapon.projectiles:
                if (
                    hasattr(projectile, "explode")
                    and projectile.age >= projectile.lifetime * 0.9
                ):
