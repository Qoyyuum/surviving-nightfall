...
class Shotgun(BaseWeapon):
    ...
    def fire(self):
        ...
        for i in range(self.pellet_count):
            angle_offset = (i - self.pellet_count / 2) * (
                self.spread_angle / self.pellet_count
            )
            angle_rad = math.radians(angle_offset)

            rotated_direction = Vec3(
                base_direction.x * math.cos(angle_rad)
                - base_direction.z * math.sin(angle_rad),
                base_direction.y,
                base_direction.x * math.sin(angle_rad)
                + base_direction.z * math.cos(angle_rad),
            ).normalized()
